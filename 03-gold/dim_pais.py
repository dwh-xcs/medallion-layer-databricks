...
# MAGIC
# MAGIC - Descrição: 
# MAGIC   - Países padronizados.
# MAGIC
# MAGIC - Objetivo: 
# MAGIC     | Campo   | Tipo   |
# MAGIC     | ------- | ------ |
# MAGIC     | cd_pais | INT    |
# MAGIC     | ds_pais | STRING |
# MAGIC
...
# Remove duplicatas.
df_dropDuplicates = df_silver.dropDuplicates()
...
if boolean_carga_full:
    ...
    (
        df_ts \
            .withColumn("cd_pais", monotonically_increasing_id()) \
            .write \
            .format("delta") \
            .mode("overwrite") \
    ...
    (
        ...
        .whenMatchedUpdate(set={
            col: f"s.{col}" 
            for col in df_ts.columns if col not in [dicionario['PK'], 'cd_pais']
        })
