...
# MAGIC
# MAGIC - Descrição: 
# MAGIC   - Produtos únicos.
# MAGIC
# MAGIC - Objetivo: 
# MAGIC   | Campo      | Tipo   |
# MAGIC   | ---------- | ------ |
# MAGIC   | cd_produto | INT    |
# MAGIC   | id_produto | STRING |
# MAGIC   | vl_produto | DOUBLE |
# MAGIC
...
orders_schema_dict = {
    ...
    "id_produto": {
        "datatype": "string",
        "description": "Identificador do produto no sistema."
    },
    "vl_produto": {
        "datatype": "double",
        "description": "Valor do produto unitário."
    },
    "dt_partition": {
        "datatype": "Date",
...
if boolean_carga_full == False:
    ...
    df_silver = (
        ...
        .select(
            ...
            col("unit_price").alias("vl_produto")
...
# Remove duplicatas.
df_dropDuplicates = df_silver.dropDuplicates(['id_produto'])
print(f"Quantidade de registros após remoção de duplicatas: {df_dropDuplicates.count()}")

# COMMAND ----------

# MAGIC %md
# MAGIC ## 04. Adicionando data e hora da carga
...
if boolean_carga_full:
    ...
    (
        df_ts \
            .withColumn("cd_produto", monotonically_increasing_id()) \
            .write \
            .format("delta") \
            .mode("overwrite") \
    ...
    (
        ...
        .whenMatchedUpdate(set={
            col: f"s.{col}" 
            for col in df_ts.columns if col not in [dicionario['PK'], 'cd_produto']
        })
...
# MAGIC %md
# MAGIC ## 06. Adiciona Comentarios

# COMMAND ----------

# MAGIC %md
# MAGIC ## 06.1 Adiciona Comentarios: Tabela
...
# MAGIC %md
# MAGIC ## 06.2 Adiciona Comentarios: Colunas
